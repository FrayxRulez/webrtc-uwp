...
using ::ABI::Windows::Media::Capture::Frames::IBufferMediaFrame;
...
HRESULT VideoCaptureWinRTInternal::StartCapture(
    ...
    for (unsigned int i = 0; SUCCEEDED(hr) && (i < supported_formats_count);
      ...
      if (SUCCEEDED(hr)) {
        ...
        if (video_type != capability.videoType &&
          ...
          RTC_LOG(LS_INFO) << "StartCapture info: wrong video type: "
                           << video_type
                           << ", requested: " << capability.videoType;
          continue;
        }
      }
      ...
      if (SUCCEEDED(hr) &&
        ...
        RTC_LOG(LS_INFO) << "StartCapture info: width or height don't match: "
                         << frame_width << "x" << frame_height
                         << ", requested: " << capability.width << "x"
                         << capability.height;
        continue;
      }
      ...
      if (SUCCEEDED(hr) && SafelyComputeMediaRatio(media_ratio.Get()) >
        ...
        RTC_LOG(LS_INFO) << "StartCapture info: fps is too high: "
                         << SafelyComputeMediaRatio(media_ratio.Get())
                         << ", requested: " << capability.maxFPS;
        continue;
      }
  ...
  if (SUCCEEDED(hr)) {
    ...
    RTC_LOG(LS_ERROR)
        << "StartCapture info: could not find any matching frame source";
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    RTC_LOG(LS_ERROR) << "StartCapture error: can't create frame source";
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    RTC_LOG(LS_ERROR) << "StartCapture error: can't start the frame reader";
  }
...
HRESULT VideoCaptureWinRTInternal::FrameArrived(
  ...
  ComPtr<IBufferMediaFrame> buffer_media_frame;
  ...
  if (SUCCEEDED(hr) && media_frame_reference) {
    ...
    if (SUCCEEDED(hr) && video_media_frame) {
      ...
      size_y = bitmap_plane_description_y.StartIndex;
      ...
      hr = media_frame_reference->get_BufferMediaFrame(&buffer_media_frame);

      if (frameInfo.videoType == VideoType::kI420) {
        size_y = frameInfo.width * frameInfo.height;
        stride_y = frameInfo.width;
        stride_uv = (frameInfo.width + 1) / 2;
      }

      if (SUCCEEDED(hr) && buffer_media_frame) {
        hr = buffer_media_frame->get_Buffer(&buffer);
      }

      if (SUCCEEDED(hr)) {
        hr = buffer->get_Length(&bitmap_capacity);
        hr = buffer.As(&buffer_byte_access);
      }

      if (SUCCEEDED(hr)) {
        hr = buffer_byte_access->Buffer(&bitmap_content);
      }
    }

    if (SUCCEEDED(hr)) {
      pfn_incoming_frame_(bitmap_content,           // Plane Y
                          stride_y,                 // Stride Y
                          bitmap_content + size_y,  // Plane UV
                          stride_uv,                // Stride UV
