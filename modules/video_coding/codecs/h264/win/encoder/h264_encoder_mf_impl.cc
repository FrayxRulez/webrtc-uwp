...
int32_t H264EncoderMFImpl::InitEncode(
  ...
  frame_dropping_on_ = codec_settings->GetFrameDropEnabled();
...
void H264EncoderMFImpl::OnH264Encoded(ComPtr<IMFSample> sample) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    encodedImage.SetEncodedData(
        EncodedImageBuffer::Create(sendBuffer.data(), curLength));
    ...
    for (uint32_t i = 0; i < sendBuffer.size() - 5; ++i) {
      ...
      if (prefixLengthFound > 0) {
        // fragmentationHeader.VerifyAndAllocateFragmentationHeader(fragIdx +
        // 1); fragmentationHeader.fragmentationOffset[fragIdx] =
        //     i + prefixLengthFound;
