...
void AddSupportedH264Codecs(vector<SdpVideoFormat>& formats) {
  formats.emplace_back(CreateH264Format(H264Profile::kProfileBaseline,
                                        H264Level::kLevel3_1, "1"));
  formats.emplace_back(CreateH264Format(H264Profile::kProfileBaseline,
                                        H264Level::kLevel3_1, "0"));
  formats.emplace_back(CreateH264Format(
...
unique_ptr<VideoDecoder> H264MFDecoderFactory::CreateVideoDecoder(
  ...
  auto test = builtin_video_decoder_factory_->CreateVideoDecoder(format);
  return test;
}
